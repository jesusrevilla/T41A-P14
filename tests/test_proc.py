...
def test_eliminar_id_exitoso(db_cursor):
    db_cursor.execute(
        "INSERT INTO productos (nombre, stock, precio) VALUES (%s, %s, %s) RETURNING id",
        ('Producto para Borrar', 1, 99.99)
    )
...
def test_aumenta_precio(db_cursor):
    db_cursor.execute(
        "INSERT INTO productos (nombre, stock, precio) VALUES (%s, %s, %s) RETURNING id",
        ('Producto para Aumentar', 1, 100.00)
    )
    producto_id = db_cursor.fetchone()[0]
...
def test_busqueda_por_rango_funciona(db_cursor):
    db_cursor.execute("""
        ...
        ('Producto Rango A', 3, 10.00),
        ('Producto Rango B', 4, 50.00),
        ('Producto Rango C', 10, 500.00);
    """)
